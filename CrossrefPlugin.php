...
class CrossrefPlugin extends GenericPlugin implements IDoiRegistrationAgency
{
    ...
    public function register($category, $path, $mainContextId = null)
    {
        ...
        if ($success) {
            ...
            if ($this->getEnabled($mainContextId)) {
                ...
                $filterGroup = $filterGroupDao->getObjectBySymbolic('monograph=>crossref-xml');
    
                if (!$filterGroup) {
                    error_log('Registrando grupo de filtros monograph=>crossref-xml en OMP');
    
                    $filterGroup = new FilterGroup();
                    $filterGroup->setSymbolic('monograph=>crossref-xml');
                    $filterGroup->setDisplayName('Crossref XML Export for Monographs');
                    $filterGroup->setDescription('Export monographs to Crossref XML format');
                    $filterGroup->setInputType('class::classes.monograph.Monograph[]');
                    $filterGroup->setOutputType('xml::schema(https://www.crossref.org/schemas/crossref5.3.1.xsd)');
    
                    $filterGroupDao->insertObject($filterGroup);
                    error_log('Grupo de filtros registrado exitosamente.');
                } else {
                    error_log('El grupo de filtros monograph=>crossref-xml ya está registrado en OMP.');
                }
    
                // 🚀 Ahora registramos el filtro en la tabla filters
                $filterDao = DAORegistry::getDAO('FilterDAO'); /** @var FilterDAO $filterDao */
                $existingFilters = $filterDao->getObjectsByGroup('monograph=>crossref-xml');
    
                if (empty($existingFilters)) {
                    error_log('Registrando filtro monograph=>crossref-xml en OMP.');
    
                    // 🔹 Extraer inputType y outputType como strings
                    $inputType = (string) $filterGroup->getInputType();
                    ...
                    // 🔹 Asegurar que `setTransformationType` reciba strings
                    $filter->setTransformationType($inputType, $outputType);
    
                    $filterDao->insertObject($filter);
                    error_log('Filtro monograph=>crossref-xml registrado exitosamente.');
                } else {
                    error_log('El filtro monograph=>crossref-xml ya estaba registrado.');
                }
            }
        }
    ...
    public function addToSchema(string $hookName, array $args): bool
    {
        ...
        foreach ($settings as $settingName) {
            ...
            error_log('addToSchema: ' . $settingName);
        }
    ...
    public function isPluginConfigured(Context $context): bool
    {
        ...
        $schemaService = app()->get(PKPSchemaService::class);
        ...
        /*
        if (!$context->getData('publisher') || !($context->getData('onlineIssn') || $context->getData('printIssn'))) {
            return false;
        }
        */
